...
variable "cloud_run_gantt_parser_name" {
  description = "Nome del Cloud Run gantt-parser-v1 (destinazione trigger Pub/Sub Gantt)"
  type        = string
}
...
variable "topic_gcs_gantt_id" {
  description = "ID del topic bt-platform-gcs-gantt-{env} (pubblicato da bucket-watcher)"
  type        = string
}
